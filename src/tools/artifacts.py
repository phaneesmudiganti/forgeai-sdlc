...
def write_artifact_files(
    ...
    def _write_category(code_text: str, category_name: str):
        """Write code artifacts for a specific category (backend, frontend, tests, devops)."""
        logger.debug(f"write_artifact_files._write_category() - processing {category_name} code ({len(code_text)} chars)")
